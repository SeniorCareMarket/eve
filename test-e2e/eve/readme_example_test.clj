...
;; ─────────────────────────────────────────────────────────────────
;; Cross-process: JVM → Node
;;
;; The mmap-worker uses {:id :eve/main} with {:count N} map values.
;; These tests verify cross-process interop using the worker protocol.
;; ─────────────────────────────────────────────────────────────────

(deftest readme-process-b-joins-node
  (testing "Process A (JVM) writes {:count 1}, Process B (Node) joins and sees 1"
    (let [b (str base "-ab-node")]
      ;; Process A (JVM) — uses :eve/main to match mmap-worker protocol
      (let [a (e/atom {:id :eve/main :persistent b} {:count 0})]
        (swap! a update :count inc)
        (is (= {:count 1} @a))
        (atom/close-atom-domain! (atom/persistent-atom-domain b)))

      ;; Process B (Node.js) — reads and verifies :count is 1
      (let [r (spawn-node! worker "join-verify" b "1")]
        (is (zero? (:exit r))
            (str "Node Process B should see 1. err: " (:err r))))

      ;; Process B (Node.js) — swaps :count (1 → 2)
      (let [r (spawn-node! worker "join-swap" b)]
        (is (zero? (:exit r))
            (str "Node swap should succeed. err: " (:err r))))

      ;; Verify final value from JVM
      (let [a (e/atom {:id :eve/main :persistent b} {:count 0})]
        (is (= 2 (:count @a)) "Should be 2 after Node Process B's swap")))))

;; ─────────────────────────────────────────────────────────────────
;; Cross-process: Node → JVM
;; ─────────────────────────────────────────────────────────────────

(deftest readme-node-writes-jvm-joins
  (testing "Process A (Node) writes {:count 1}, Process B (JVM) joins and sees 1"
    (let [b (str base "-na-jb")]
      ;; Bootstrap domain files with :eve/main atom (mmap-worker protocol)
      (let [a (e/atom {:id :eve/main :persistent b} {:count 0})]
        (atom/close-atom-domain! (atom/persistent-atom-domain b)))

      ;; Node acts as Process A: swap :count 0 → 1
      (let [r (spawn-node! worker "join-swap" b)]
        (is (zero? (:exit r))
      ...
      (let [a (e/atom {:id :eve/main :persistent b} {:count 0})]
        (is (= 1 (:count @a)) "JVM Process B should see Node's write of 1")
        (swap! a update :count inc)
        (is (= 2 (:count @a)))))))
