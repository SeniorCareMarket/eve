   ...
   (deftest dataset-construction-test
     (testing "Dataset from column map (inside atom)"
       (let [ea (make-eve-atom nil)
             ds1 (swap! ea (fn [_]
                             (ds/dataset {:price (e/eve-array :float64 [10.5 20.3 30.1])
...
#?(:cljs
   (deftest dtypes-test
     (testing "dtypes returns type map"
       (let [ea (make-eve-atom nil)
             ds1 (swap! ea (fn [_]
                             (ds/dataset {:price (e/eve-array :float64 [1.0 2.0])
   ...
   (deftest row-count-test
     (testing "row-count"
       (let [ea (make-eve-atom nil)]
         (is (= 5 (ds/row-count
                     (swap! ea (fn [_]
                                 (ds/dataset {:a (e/eve-array :int32 [1 2 3 4 5])}))))))))))
...
#?(:cljs
   (deftest select-columns-test
     (testing "select-columns"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:a (e/eve-array :int32 [1 2])
   ...
   (deftest add-column-test
     (testing "add-column"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:a (e/eve-array :int32 [1 2])})]
                               (ds/add-column ds1 :b (e/eve-array :int32 [3 4])))))]
...
#?(:cljs
   (deftest rename-columns-test
     (testing "rename-columns"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:price (e/eve-array :float64 [10.5 20.3])
                                                    :qty   (e/eve-array :int32 [100 200])})]
...
#?(:cljs
   (deftest filter-rows-test
     (testing "filter-rows with predicate"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:price (e/eve-array :float64 [10.5 20.3 5.0 30.1])
...
#?(:cljs
   (deftest sort-by-column-test
     (testing "sort ascending"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:v (e/eve-array :int32 [30 10 20])})]
...
#?(:cljs
   (deftest head-tail-slice-test
     (testing "head"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:v (e/eve-array :int32 [1 2 3 4 5])})]
                               (ds/head ds1 3))))]
     ...
     (testing "slice"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:v (e/eve-array :int32 [10 20 30 40 50])})]
                               (ds/slice ds1 1 4))))]
         (is (= 3 (ds/row-count ds2)))
...
(deftest sum-mean-test
  (testing "sum and mean"
    (let [col (e/eve-array :float64 [10.0 20.0 30.0])]
      (is (approx= 60.0 (col/sum col) 0.001))
      (is (approx= 20.0 (col/mean col) 0.001)))))

(deftest min-max-val-test
  (testing "min-val and max-val"
    (let [col (e/eve-array :int32 [30 10 20 50 5])]
      (is (= 5 (col/min-val col)))
      (is (= 50 (col/max-val col))))))
...
(deftest comparison-ops-test
  (testing "gt, lt, eq return uint8 mask"
    (let [col (e/eve-array :int32 [10 20 30 40 50])
          mask-gt (col/gt col 25)
          mask-lt (col/lt col 25)
          mask-eq (col/eq col 30)]
...
(deftest take-indices-reindex-test
  (testing "take-indices"
    (let [col (e/eve-array :int32 [10 20 30 40 50])
          ...
          result (col/take-indices col idx)]
      (is (= 3 (count result)))
      (is (= 50 (nth result 0)))
      (is (= 30 (nth result 1)))
      (is (= 10 (nth result 2)))))
  #?(:cljs
     (testing "reindex dataset"
       (let [ea (make-eve-atom nil)
             ds2 (swap! ea (fn [_]
                             (let [ds1 (ds/dataset {:v (e/eve-array :int32 [10 20 30])})
...
#?(:cljs
   (deftest multi-dtype-test
     (testing "dataset with multiple dtypes"
       (let [ea (make-eve-atom nil)
             ds1 (swap! ea (fn [_]
                             (ds/dataset {:id    (e/eve-array :int32 [1 2 3])
...
#?(:cljs
   (deftest dataset-lookup-test
     (testing "Dataset implements ILookup"
       (let [ea (make-eve-atom nil)
             ds1 (swap! ea (fn [_]
                             (ds/dataset {:a (e/eve-array :int32 [1 2 3])})))]
         (is (some? (:a ds1)))
         (is (some? (ds1 :a)))
         (is (nil? (:missing ds1)))))))
