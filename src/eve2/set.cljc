(ns eve2.set
  ...
  (:require
   ...
   #?@(:cljs [[eve.deftype-proto.alloc :as eve-alloc]]
       :clj  [[eve.mem :as mem :refer [eve-bytes->value value->eve-bytes
                                        value+sio->eve-bytes
...
(defn- serialize-val-bytes
  "Serialize a set element to bytes."
  [v]
...
(defn- make-single-value-node!
  "Create bitmap node with exactly 1 inline value."
  [sio data-bm vb]
  (let [vlen (bytes-length vb)
...
(defn- make-two-value-node!
  "Create bitmap node with exactly 2 inline values."
  [sio data-bm vb1 vb2]
...
(defn- make-single-child-node!
  "Create bitmap node with 0 inline values and 1 child."
  [sio node-bm child-off]
...
(defn- make-child-and-value-node!
  "Create bitmap node with 1 inline value and 1 child."
  [sio data-bm node-bm child-off vb]
  (let [vlen (bytes-length vb)
...
(defn- make-collision-node!
  "Create collision node from entries: [[vb] ...]."
  [sio vh entries]
  (let [cnt (count entries)
    ...
    (-sio-write-i32! sio node-off 8 0) ;; padding
    (reduce (fn [pos [vb]]
...
(defn- node-values-size
  "Compute total byte size of inline values in a bitmap node."
...
(defn- copy-node-patch-child!
  "Bulk-copy a bitmap node and patch one child pointer."
...
(defn- hamt-find
  "Check if value v is in the set. Returns boolean."
  [sio root-off v vh vb]
...
(defn- hamt-conj
  "Add value to set. Returns [new-root added?]."
  [sio root-off vh vb shift]
    ...
    (let [nt (-sio-read-u8 sio root-off 0)]
      (case (int nt)
            ...
            (cond
              ...
              (let [di (get-index data-bm bit)
                ...
                (if (value-matches? sio root-off pos vb)
                  ;; Already present
                  [root-off false]
                  ;; Different value — push down
                  (let [[existing-vb _] (read-value-at sio root-off pos)
                        existing-vh vh] ;; same hash bucket
                    (if (>= shift 30)
                      ;; Collision
                      ...
                      (let [sub-shift (+ shift SHIFT_STEP)
                        ...
                        ;; Since both have same hash at this level, they land on same bit
                        ;; Need to recursively push down
              ...
              ;; Empty position — add inline value
              :else
              (let [di (get-index data-bm bit)
        ...
        2 (let [node-hash (-sio-read-i32 sio root-off 4)
            ...
            (if (== vh node-hash)
              ;; Same hash — check if already present, else add
              ...
              (if (>= shift 30)
                ;; Can't split further, add to collision anyway
                (let [entries (loop [i 0 pos COLLISION_HEADER_SIZE acc []]
                                (if (>= i cc) acc
                                  (let [[ev np] (read-value-at sio root-off pos)]
                                    (recur (inc i) np (conj acc ev)))))]
...
;;=============================================================================
;; HAMT Disj (unified) — returns [new-root removed?]
;;=============================================================================

(defn- hamt-disj
  "Remove value from set. Returns [new-root removed?]."
  [sio root-off vh vb shift]
  (if (== root-off NIL_OFFSET)
    [root-off false]
    (let [nt (-sio-read-u8 sio root-off 0)]
...
(defn- hamt-val-reduce
  "Walk HAMT tree, calling (f acc v) at each entry."
  [sio root-off f init]
  (if (== root-off NIL_OFFSET)
    init
    (let [nt (-sio-read-u8 sio root-off 0)]
...
(defn- hamt-seq
  "Build a seq of values from HAMT."
  [sio root-off]
...
(defn- write-set-header!
  "Allocate and write a 12-byte EveHashSet header."
  [sio cnt root-off]
...
(defn- read-set-header
  "Read cnt and root-off from header. Returns [cnt root-off]."
  [sio header-off]
...
#?(:cljs
   (deftype EveHashSet [cnt root-off header-off
     ...
     (-seq [_]
       (when (pos? cnt)
         (hamt-seq eve-alloc/cljs-sio root-off)))
   ...
   :clj
   (deftype EveHashSet
     [^long cnt ^long root-off ^long header-off sio _meta]
     ...
     clojure.lang.IObj
     (withMeta [_ new-meta]
       (EveHashSet. cnt root-off header-off sio new-meta))
...
(defn- make-eve2-hash-set
  ...
  (let [hdr (write-set-header! sio cnt root-off)]
    #?(:cljs (EveHashSet. cnt root-off hdr nil)
...
(defn eve2-hash-set-from-header
  ...
  (let [[cnt root-off] (read-set-header sio header-off)]
    #?(:cljs (EveHashSet. cnt root-off header-off nil)
...
(defn hash-set
  ...
  (let [sio (get-sio)]
    (reduce (fn [s v]
              (let [^bytes vb (serialize-val-bytes v)
                    vh (portable-hash-bytes vb)
                    s-cnt #?(:cljs (.-cnt s) :clj (.cnt ^EveHashSet s))
                    s-root #?(:cljs (.-root-off s) :clj (.root_off ^EveHashSet s))
                    [new-root added?] (hamt-conj sio s-root vh vb 0)]
                (if-not added?
                  s
                  (make-eve2-hash-set sio (inc s-cnt) new-root))))
            (make-eve2-hash-set sio 0 NIL_OFFSET)
            vals)))
