...
#?(:cljs
   (deftype EveList [cnt head-off header-off]
   ...
   :clj
   (deftype EveList
     [^long cnt ^long head-off ^long header-off sio _meta]
     ...
     clojure.lang.IObj
     (withMeta [_ new-meta]
       (EveList. cnt head-off header-off sio new-meta))
...
(defn eve2-list-from-header
  ...
  (let [[cnt head-off] (read-list-header sio header-off)]
    #?(:cljs (EveList. cnt head-off header-off)
...
(defn eve2-list
  ...
  (let [sio (get-sio)]
    (reduce (fn [lst v]
              (let [vb (serialize-element-bytes v)
                    lst-head #?(:cljs (.-head-off lst) :clj (.head_off ^EveList lst))
                    lst-cnt  #?(:cljs (.-cnt lst) :clj (.cnt ^EveList lst))
                    node-off (alloc-list-node! sio lst-head vb)]
                (make-eve2-list sio (inc lst-cnt) node-off)))
            (make-eve2-list sio 0 NIL_OFFSET)
            (reverse coll))))
